...
void main() {
  group('PrinterService ESC/POS Formatting & Socket Output Tests', () {
    ...
    test('testConnection sends initial commands, test labels and paper cut', () async {
      ...
      expect(mockSocket.writtenBytes.sublist(0, 2), equals(EscPosCommands.init));
      ...
      expect(_containsBytes(mockSocket.writtenBytes, [89, 97, 122, 141, 99, 141]), isTrue); // 'Yazıcı'
    });

    test('printSaleReceipt outputs formatted receipt with items, totals and QR code', () async {
      ...
      expect(_containsBytes(mockSocket.writtenBytes, [83, 129, 116, 32, 40, 51, 32, 120, 32, 51, 48, 46, 48, 48, 41]), isTrue); // 'Süt (3 x 30.00)'
      expect(writtenText, contains('Elma (2 x 10.00)'));
      
      // Verify totals exist
      expect(writtenText, contains('TOPLAM: 120.00 TL'));
      expect(_containsBytes(mockSocket.writtenBytes, [153, 100, 101, 110, 101, 110, 58, 32, 56, 48, 46, 48, 48, 32, 84, 76]), isTrue); // 'Ödenen: 80.00 TL'
      expect(_containsBytes(mockSocket.writtenBytes, [75, 97, 108, 97, 110, 32, 66, 111, 114, 135, 58, 32, 52, 48, 46, 48, 48, 32, 84, 76]), isTrue); // 'Kalan Borç: 40.00 TL'
      ...
      expect(mockSocket.writtenBytes.sublist(len - 4), equals(EscPosCommands.cut));
    });

    test('printOrderReceipt formats detailed receipt with items, totals and QR code', () async {
      ...
      await printerService.printOrderReceipt(order, items, null, testSettings, paidAmount: 30.0);

      final writtenText = String.fromCharCodes(mockSocket.writtenBytes);
      expect(_containsBytes(mockSocket.writtenBytes, [42, 42, 42, 32, 83, 152, 80, 65, 82, 152, 158, 32, 70, 152, 158, 152, 32, 42, 42, 42]), isTrue); // '*** SİPARİŞ FİŞİ ***'
      ...
      expect(_containsBytes(mockSocket.writtenBytes, [153, 100, 101, 110, 101, 110, 58, 32, 51, 48, 46, 48, 48, 32, 84, 76]), isTrue); // 'Ödenen: 30.00 TL'
      expect(_containsBytes(mockSocket.writtenBytes, [75, 97, 108, 97, 110, 32, 66, 111, 114, 135, 58, 32, 55, 48, 46, 48, 48, 32, 84, 76]), isTrue); // 'Kalan Borç: 70.00 TL'
      expect(writtenText, contains('order|order-test-id-999'));
    });

    test('printXReport outputs correct Ara Rapor layout and categories', () async {
      ...
      final writtenText = String.fromCharCodes(mockSocket.writtenBytes);
      expect(_containsBytes(mockSocket.writtenBytes, [71, 154, 78, 32, 152, 128, 152, 32, 88, 32, 82, 65, 80, 79, 82, 85]), isTrue); // 'GÜN İÇİ X RAPORU'
      expect(writtenText, contains('Toplam Ciro:'));
      expect(writtenText, contains('500.00 TL'));
      expect(_containsBytes(mockSocket.writtenBytes, [83, 97, 116, 141, 159, 32, 83, 97, 121, 141, 115, 141, 58]), isTrue); // 'Satış Sayısı:'
      expect(writtenText, contains('4'));
      expect(_containsBytes(mockSocket.writtenBytes, [71, 141, 100, 97, 32, 40, 55, 48, 37, 41]), isTrue); // 'Gıda (70%)'
    ...
    test('printZReport outputs final daily closeout slip', () async {
      ...
      final writtenText = String.fromCharCodes(mockSocket.writtenBytes);
      expect(_containsBytes(mockSocket.writtenBytes, [71, 154, 78, 32, 83, 79, 78, 85, 32, 90, 32, 82, 65, 80, 79, 82, 85]), isTrue); // 'GÜN SONU Z RAPORU'
      expect(writtenText, contains('Z No:'));
      expect(writtenText, contains('TOPLAM TAHSILAT:'));
      expect(writtenText, contains('800.00 TL'));
      expect(_containsBytes(mockSocket.writtenBytes, [71, 154, 78, 32, 83, 79, 78, 85, 32, 75, 65, 80, 65, 78, 77, 73, 158, 84, 73, 82]), isTrue); // 'GÜN SONU KAPANMIŞTIR'
    });

    test('printCollectionReceipt formats collection receipt correctly', () async {
      ...
      final writtenText = String.fromCharCodes(mockSocket.writtenBytes);
      expect(_containsBytes(mockSocket.writtenBytes, [84, 65, 72, 83, 152, 76, 65, 84, 32, 77, 65, 75, 66, 85, 90, 85]), isTrue); // 'TAHSİLAT MAKBUZU'
      expect(writtenText, contains('Ahmet Yilmaz'));
      expect(writtenText, contains('Nakit')); // 'cash' translated to Nakit by _getPaymentLabel
      ...
      expect(_containsBytes(mockSocket.writtenBytes, [75, 97, 108, 97, 110, 32, 66, 111, 114, 135, 58, 32, 54, 48, 46, 53, 48, 32, 84, 76]), isTrue);
    });

    test('printSaleReceipt and printOrderReceipt display customer debt and credit correctly', () async {
      final customer = CustomerEntity(
        id: 'cust-test',
        name: 'Ahmet Yilmaz',
        phone: '05551234567',
      ...
      final writtenText = String.fromCharCodes(mockSocket.writtenBytes);
      expect(writtenText, contains('Ahmet Yilmaz'));
      expect(_containsBytes(mockSocket.writtenBytes, [71, 101, 135, 109, 105, 159, 32, 66, 111, 114, 135, 58, 32, 50, 53, 48, 46, 53, 48, 32, 84, 76]), isTrue); // 'Geçmiş Borç: 250.50 TL'
    });
  });
}
