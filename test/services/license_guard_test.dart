...
void main() {
  group('LicenseGuard Tests', () {
    ...
    test('Bypasses validation if trial is active', () async {
      ...
      await prefs.setString('serenut_trial_checksum', 'invalid_but_ignored_due_to_initialization');
    ...
    test('Throws DEVICE503 if clock manipulation is detected', () async {
      // Expire trial first
      await prefs.setInt('nutopiano_first_launch_timestamp', DateTime.now().subtract(const Duration(days: 40)).millisecondsSinceEpoch);
      await trialManager.initTrialIfNeeded();

      // Configure a valid license token and trigger clock integrity failure
      await prefs.setString('license_token', 'eyJtZXJjaGFudElkIjoiMSIsImFsbG93ZWREZXZpY2VzIjpbIioiXSwiZXhwaXJ5RGF0ZSI6IjIwMzAtMDEtMDFUMDA6MDA6MDAuMDAwWiIsInRpZXIiOiJQUk8iLCJmZWF0dXJlcyI6W10sInNpZ25hdHVyZSI6IiJ9');
    ...
    test('Enforces 72-hour offline grace check', () async {
      ...
      await prefs.setString('license_last_verified_at', DateTime.now().subtract(const Duration(hours: 2)).toIso8601String());
      await expectLater(licenseGuard.verifyAccess(), completes);

      // Scenario C: Verified online 75 hours ago (exceeded 72 hours)
      await prefs.setString('license_last_verified_at', DateTime.now().subtract(const Duration(hours: 75)).toIso8601String());
      try {
        await licenseGuard.verifyAccess();
