...
void main() {
  ...
  group('Offline Settings Sync Tests', () {
    ...
    test(
        'Retries pending company settings patch at the start of bootstrap sync',
        () async {
      ...
      apiClient.mockHandler = (request) {
        if (request.url.path.contains('/api/v1/company') &&
    ...
    test(
        'Sends expected_version and updates local version in business_profile on successful sync settings PATCH',
        () async {
      ...
      apiClient.mockHandler = (request) {
        if (request.url.path.contains('/api/v1/company') &&
          ...
          return const ApiResponse(
            statusCode: 200,
            body:
                '{"id": 1, "name": "Updated Market name", "owner_name": "Owner Name", "version": 6}',
            headers: {},
          );
        }
        if (request.url.path.contains('/sync/bootstrap/company') ||
      ...
      final rows =
          await db.query('business_profile', where: 'id = ?', whereArgs: [1]);
      expect(rows.first['version'], 6);
      expect(rows.first['name'], 'Updated Market name');
    });

    test(
        'Stops retrying and logs when version conflict (409 Conflict) is returned from server',
        () async {
      await db.insert('settings', {
