  ...
  group('Zip Bomb & Import Cleanup Tests', () {
    test('Rejects zip archives larger than 100MB', () async {
      ...
      expect(
        ...
        throwsA(predicate(
            (e) => e.toString().contains('Dosya boyutu sınırlandırılmıştır'))),
      );
    ...
    test(
        'Rejects zip archives with uncompressed sizes larger than 100MB (Zip Bomb)',
        () async {
      ...
      expect(
        ...
        throwsA(predicate((e) =>
            e.toString().contains('Yüksek sıkıştırma oranı tespit edildi'))),
      );
    });

    test(
        'Proof of lazy decompression: decodeBytes does not allocate uncompressed content',
        () async {
      final archive = Archive();
