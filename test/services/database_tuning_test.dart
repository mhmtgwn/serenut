...
void main() {
  ...
  group('SQLite Tuning, Index usage & Concurrency Hardening Tests', () {
    ...
    test('SQLite Explain Query Plan — Verify indexes are used', () async {
      ...
      final explainSales = await db.rawQuery(
          'EXPLAIN QUERY PLAN SELECT * FROM sales WHERE is_synced = 0');
      final salesPlan =
          explainSales.map((r) => r['detail'].toString()).join(' | ');
      expect(salesPlan, contains('idx_sales_synced'));

      // 2. Verify idx_sale_items_product on sale_items(product_id)
      final explainItems = await db.rawQuery(
          'EXPLAIN QUERY PLAN SELECT * FROM sale_items WHERE product_id = "abc"');
      final itemsPlan =
          explainItems.map((r) => r['detail'].toString()).join(' | ');
      expect(itemsPlan, contains('idx_sale_items_product'));

      // 3. Verify idx_ft_reference on financial_transactions(reference_id)
      final explainFt = await db.rawQuery(
          'EXPLAIN QUERY PLAN SELECT * FROM financial_transactions WHERE reference_id = "ft_abc"');
      ...
      final explainOrders = await db.rawQuery(
          'EXPLAIN QUERY PLAN SELECT * FROM orders WHERE customer_id = "cust_abc"');
      final ordersPlan =
    ...
    test('SQLite Concurrency Simulation — Simultaneous imports and sales flow',
      ...
      final productCount = (await db.rawQuery('SELECT COUNT(*) FROM products'))
          ...
          .values
          .first as int;
      final saleCount = (await db.rawQuery('SELECT COUNT(*) FROM sales'))
          .first
          .values
          .first as int;
