...
void main() {
  ...
  group('ReleaseManagerService.checkForUpdates', () {
    test('returns noUpdate when server returns hasUpdate=false', () async {
      ...
      final svc = ReleaseManagerService(config: testConfig, httpClient: mockClient);
    ...
    test('returns noUpdate gracefully on server error (non-throwing)', () async {
      final errorClient = MockClient((_) async => http.Response('Internal Error', 500));

      final svc = ReleaseManagerService(config: testConfig, httpClient: errorClient);
      final result = await svc.checkForUpdates(
        currentVersion: '1.0.0+1',
        platform: 'android',
      );
    ...
    test('returns noUpdate gracefully on network failure (non-throwing)', () async {
      final failClient = MockClient((_) async => throw const SocketException('No route to host'));

      final svc = ReleaseManagerService(config: testConfig, httpClient: failClient);
      final result = await svc.checkForUpdates(
        currentVersion: '1.0.0+1',
        platform: 'android',
      );
    ...
    test('includes correct query params in request (platform, channel, device_id)', () async {
      String? capturedUrl;
      final capturingClient = MockClient((request) async {
        capturedUrl = request.url.toString();
        return http.Response(
          jsonEncode({'hasUpdate': false, 'isForceUpdate': false, 'latestVersion': '1.0', 'channel': 'stable'}),
          200,
        );
      });

      final svc = ReleaseManagerService(config: testConfig, httpClient: capturingClient);
  ...
  group('ReleaseManagerService.verifyDownload', () {
    ...
    test('returns false when SHA-256 does NOT match', () async {
      final svc = ReleaseManagerService(config: testConfig);
      final tempFile = File('${Directory.systemTemp.path}/test_verify_fail.bin');
      await tempFile.writeAsBytes([0xDE, 0xAD, 0xBE, 0xEF]);

      final isValid = await svc.verifyDownload(tempFile, 'wrong_hash_value', '');
