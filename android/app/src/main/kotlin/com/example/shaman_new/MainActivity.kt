package com.example.shaman_new
...
class MainActivity : FlutterActivity() {
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, SCANNER_CHANNEL).setMethodCallHandler { call, result ->
            when (call.method) {
                "hasScanner" -> {
                    ...
                    // Gerçek donanım tespiti burada yapılmalı, şimdilik true döndürüyoruz
                    result.success(true)
                }
                "getScannerInfo" -> {
                ...
                "startScan" -> {
                    Log.d("SunmiScanner", "Barkod tarama başlatıldı")
                    // Tarama başlatıldı, 2 saniye sonra simüle edilmiş bir barkod döndürelim
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, NFC_CHANNEL).setMethodCallHandler { call, result ->
            when (call.method) {
                "hasNfc" -> {
                    Log.d("SunmiNFC", "hasNfc çağrıldı")
                    // Gerçek donanım tespiti burada yapılmalı, şimdilik false döndürelim (test için)
                    result.success(false)
                }
                "getNfcInfo" -> {
                    val info = HashMap<String, String>()
                    info["model"] = "Sunmi NFC V2"
                    info["version"] = "2.0"
                    result.success(info)
                }
                else -> {
        ...
        // Para çekmecesi kanalı
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, DRAWER_CHANNEL).setMethodCallHandler { call, result ->
            when (call.method) {
                "hasDrawer" -> {
                    Log.d("SunmiDrawer", "hasDrawer çağrıldı")
                    // Gerçek donanım tespiti burada yapılmalı, şimdilik false döndürelim (test için)
                    result.success(false)
                }
                "getDrawerInfo" -> {
                    val info = HashMap<String, String>()
                    info["model"] = "Sunmi Cash Drawer V2"
                    info["version"] = "2.0"
                    result.success(info)
                }
                else -> {
    ...
    // Sunmi yazıcısına bağlanma fonksiyonu
    private fun connectPrinter() {
        try {
            Log.d("SunmiPrinter", "Yazıcıya bağlanılıyor...")
