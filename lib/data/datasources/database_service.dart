...
class DatabaseService {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 5) {
      ...
      final deviceColumnNames =
          deviceTableInfo.map((row) => row['name'] as String).toList();

      if (!deviceColumnNames.contains('isReceiptPrinter')) {
        await db.execute(
            'ALTER TABLE devices ADD COLUMN isReceiptPrinter INTEGER DEFAULT 0');
      }

      if (!deviceColumnNames.contains('isLabelPrinter')) {
        await db.execute(
            'ALTER TABLE devices ADD COLUMN isLabelPrinter INTEGER DEFAULT 0');
      }
      ...
      try {
        ...
        for (var assignment in printerAssignments) {
          ...
          if (type == 'receipt') {
            await db.update('devices', {'isReceiptPrinter': 1},
                where: 'id = ?', whereArgs: [deviceId]);
          } else if (type == 'label') {
            await db.update('devices', {'isLabelPrinter': 1},
                where: 'id = ?', whereArgs: [deviceId]);
          }
        }
  ...
  Future<List<Map<String, dynamic>>> getAllCustomers() async {
    final db = await database;
    return await db.query('customers', orderBy: 'displayName COLLATE UNICODE');
  }

  Future<List<Map<String, dynamic>>> getCustomerByPhone(String phone) async {
    final db = await database;
    return await db.query('customers', where: 'phone = ?', whereArgs: [phone]);
  }
  ...
  Future<int> updateCustomer(Map<String, dynamic> customer) async {
    ...
    return await db.update('customers', customer,
  ...
  Future<int> saveDevice(Map<String, dynamic> device) async {
    ...
    debugPrint(
        'Cihaz kaydediliyor: ${deviceData['name']}, bluetoothAddress: ${deviceData['bluetoothAddress']}');
  ...
  Future<List<Map<String, dynamic>>> getAllDevices() async {
    ...
    try {
      ...
      for (var device in devices) {
        debugPrint(
            'DB Cihaz: ${device['name']}, ID: ${device['id']}, Type: ${device['type']}');
      }
      return devices;
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> getPrinters() async {
    final db = await database;
    return await db.query('devices', where: 'type = ?', whereArgs: ['printer']);
  }

  Future<Map<String, dynamic>?> getDeviceById(String id) async {
    final db = await database;
    final results =
        await db.query('devices', where: 'id = ?', whereArgs: [id], limit: 1);
  ...
  Future<int> assignReceiptPrinter(String deviceId) async {
    ...
    await db.update('devices', {'isReceiptPrinter': 0},
  ...
  Future<int> assignLabelPrinter(String deviceId) async {
    ...
    await db.update('devices', {'isLabelPrinter': 0},
  ...
  Future<Map<String, dynamic>?> getReceiptPrinter() async {
    final db = await database;
    final results =
        await db.query('devices', where: 'isReceiptPrinter = 1', limit: 1);
  ...
  Future<Map<String, dynamic>?> getLabelPrinter() async {
    final db = await database;
    final results =
        await db.query('devices', where: 'isLabelPrinter = 1', limit: 1);
