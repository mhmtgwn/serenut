...
extension IdShortener on String {
  ...
  String get toShortId {
    ...
    if ((lower.startsWith('sale-') || lower.startsWith('sale_')) && length >= 5) {
    ...
    } else if ((lower.startsWith('order-') || lower.startsWith('order_') || lower.startsWith('ord-') || lower.startsWith('ord_')) && length >= 4) {
      final idx = indexOf(RegExp(r'[-_]'));
      final suffix = substring(idx + 1);
      return 'O-${suffix.substring(suffix.length > 6 ? suffix.length - 6 : 0).toUpperCase()}';
    } else if ((lower.startsWith('trans-') || lower.startsWith('trans_')) && length >= 6) {
      final idx = indexOf(RegExp(r'[-_]'));
      final suffix = substring(idx + 1);
