...
class _SmsHistoryPageState extends ConsumerState<SmsHistoryPage> {
  String _searchQuery = '';
  String _selectedStatus = 'all'; // 'all' | 'queued' | 'sent' | 'failed' | 'retrying'
          ...
          FutureBuilder<CompanyCredits>(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                return const LinearProgressIndicator(minHeight: 2, valueColor: AlwaysStoppedAnimation(_kBlue));
              }
              ...
              return Container(
                ...
                child: Row(
                  children: [
                    Expanded(
                      child: _buildCreditCard('SMS Kredi', '${credits.smsCredits}', Icons.sms_rounded, Colors.green),
                    ),
                    const SizedBox(width: 8),
                    Expanded(
                      child: _buildCreditCard('WhatsApp', '${credits.whatsappCredits}', Icons.chat_rounded, Colors.teal),
                    ),
                    const SizedBox(width: 8),
                    Expanded(
                      child: _buildCreditCard('E-Mail', '${credits.emailCredits}', Icons.mail_rounded, Colors.purple),
                    ),
                  ],
                ),
              );
            ...
            child: FutureBuilder<List<QueueEntry>>(
              ...
              builder: (context, snapshot) {
                ...
                if (snapshot.hasError) {
                  return Center(child: Text('Kuyruk yüklenemedi: ${snapshot.error}'));
                }
                ...
                if (_searchQuery.isNotEmpty) {
                  list = list.where((e) => e.recipient.contains(_searchQuery) || e.body.toLowerCase().contains(_searchQuery)).toList();
                }
                ...
                if (list.isEmpty) {
                  return const Center(
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.mark_email_unread_rounded, size: 48, color: Colors.grey),
                        SizedBox(height: 12),
                        Text('Mesaj geçmişi bulunmuyor.', style: TextStyle(color: Colors.grey)),
                      ],
                    ),
                  );
                }
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                              ...
                              children: [
                                Row(
                                  children: [
                                    ...
                                    const SizedBox(width: 8),
                                    Text(entry.recipient, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                                  ],
                                ),
                            ...
                            const SizedBox(height: 8),
                            Text(entry.body, style: const TextStyle(fontSize: 12, color: _kTextPrimary)),
                            const SizedBox(height: 8),
                            Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(value, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: _kTextPrimary)),
        ],
      ),
    );
  }
  ...
  Widget _buildFilterChip(String label, String value) {
    ...
    return ChoiceChip(
      label: Text(label, style: TextStyle(fontSize: 11, color: isSelected ? Colors.white : _kTextSecondary)),
      selected: isSelected,
  ...
  Future<void> _showCampaignWizard() async {
    ...
    try {
      ...
      await notifRepo.saveTemplate(
        ...
        channel: 'sms',
        body: 'Merhaba {{customer}}, magazamiza gosterdiginiz ilgi icin tesekkur ederiz! {{store}}',
      );
    } catch (_) {}
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setDialogState) {
            return AlertDialog(
              title: const Text('Toplu Kampanya Sihirbazı', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              content: Column(
                mainAxisSize: MainAxisSize.min,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Alıcı Segmenti:', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
                  DropdownButton<String>(
                    value: selectedSegment,
                    isExpanded: true,
                    items: const [
                      DropdownMenuItem(value: 'all_customers', child: Text('Tüm Müşteriler', style: TextStyle(fontSize: 13))),
                      DropdownMenuItem(value: 'debtors', child: Text('Veresiye Borcu Olanlar', style: TextStyle(fontSize: 13))),
                      DropdownMenuItem(value: 'inactive_30d', child: Text('30 Gündür Alışveriş Yapmayanlar', style: TextStyle(fontSize: 13))),
                    ],
                    onChanged: (val) {
                      if (val != null) setDialogState(() => selectedSegment = val);
                    },
                  ),
                  const SizedBox(height: 12),
                  const Text('Gönderim Kanalı:', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
                  DropdownButton<String>(
                    value: selectedChannel,
                    isExpanded: true,
                    items: const [
                      DropdownMenuItem(value: 'sms', child: Text('SMS', style: TextStyle(fontSize: 13))),
                      DropdownMenuItem(value: 'whatsapp', child: Text('WhatsApp', style: TextStyle(fontSize: 13))),
                  ...
                  const SizedBox(height: 12),
                  const Text('Kullanılacak Şablon:', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
                  DropdownButton<String>(
                    value: templateName,
                    isExpanded: true,
                    items: const [
                      DropdownMenuItem(value: 'sale_invoice_sms', child: Text('Tanıtım & Teşekkür Şablonu', style: TextStyle(fontSize: 13))),
                    ],
                    onChanged: (val) {
                    ...
                    try {
                      ...
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('Kampanya başlatıldı! $queuedCount mesaj kuyruğa eklendi.')),
                        );
                      }
                    } catch (e) {
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('Kampanya tetikleme hatası: $e')),
                        );
                      }
                    } finally {
                      setState(() {
