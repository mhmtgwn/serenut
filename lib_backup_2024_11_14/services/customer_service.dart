...
class CustomerService {
  ...
  Future<void> _upgradeDb(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 2) {
      ...
      await db.execute('ALTER TABLE customers ADD COLUMN credit_balance REAL DEFAULT 0.0');
    }
    ...
    if (oldVersion < 3) {
      ...
      try {
        ...
        await db.execute('UPDATE customers SET created_at = ?, updated_at = ? WHERE created_at IS NULL OR updated_at IS NULL', [now, now]);
      } catch (e) {
  ...
  Future<double> getCustomerTotalDebt(int customerId) async {
    try {
      ...
      if (customerData.isNotEmpty && customerData.first['credit_balance'] != null) {
        final creditBalance = customerData.first['credit_balance'] as double? ?? 0.0;
  ...
  Future<bool> reduceCustomerDebt(int customerId, double amount) async {
    try {
      ...
      for (var order in orders) {
        ...
        double paymentForThisOrder = orderDebt <= remainingAmount ? orderDebt : remainingAmount;
