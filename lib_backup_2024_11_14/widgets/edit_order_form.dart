...
class _EditOrderFormState extends State<EditOrderForm> {
  ...
  Future<void> _loadPaymentHistory() async {
    ...
    try {
      final orderService = OrderService.instance;
      final payments = await orderService.getPaymentHistory(widget.orderId);
  ...
  Future<void> _updateOrder() async {
    ...
    try {
      ...
      if (paymentText.isNotEmpty && _orderStatus == 'Satış') {
        ...
        if (newPayment > 0) {
          // Bekleyen ödeme ekle
          OrderService.instance.addPendingPayment(widget.orderId, newPayment, _paymentMethod);
          // Ödemeyi hemen onayla
          await OrderService.instance.confirmPendingPayments(widget.orderId);
