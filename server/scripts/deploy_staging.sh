...
# Serenut OS — Automated Staging Deployment Playbook
# SRE, SRE-Hardening & PM2/Nginx/Postgres Bootstrapper for Staging

# Exit immediately if any command returns a non-zero status
set -e

echo "=================================================="
echo "🌿 Starting Serenut OS Staging Deployment Playbook"
echo "=================================================="
...
sudo -u postgres psql -c "CREATE DATABASE serenut_db_staging;" || true
sudo -u postgres psql -c "CREATE USER serenut_staging_user WITH ENCRYPTED PASSWORD 'SerenutStagingPass123!';" || true
sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE serenut_db_staging TO serenut_staging_user;" || true
sudo -u postgres psql -d serenut_db_staging -c "GRANT ALL ON SCHEMA public TO serenut_staging_user;" || true
...
TARGET_DIR="/var/www/serenut-api-staging"
mkdir -p ${TARGET_DIR}
...
PORT=3001
NODE_ENV=staging
DATABASE_URL=postgresql://serenut_staging_user:SerenutStagingPass123!@127.0.0.1:5432/serenut_db_staging
JWT_SECRET=staging_secret_key_32_characters_long
REFRESH_SECRET=staging_refresh_secret_key_32_characters_long
REDIS_URL=redis://127.0.0.1:6379/1
SMTP_HOST=smtp.mailtrap.io
SMTP_PORT=2525
SMTP_USER=mock
SMTP_PASS=mock
DOMAIN=staging.serenut.com
COOKIE_DOMAIN=.staging.serenut.com
...
if pm2 list | grep -q "serenut-backend-staging"; then
...
cp ${TARGET_DIR}/nginx.staging.conf /etc/nginx/sites-available/serenut-staging
ln -sf /etc/nginx/sites-available/serenut-staging /etc/nginx/sites-enabled/
...
# certbot --nginx -d staging.serenut.com -d api.staging.serenut.com --non-interactive --agree-tos -m devops@serenut.com
...
echo "=================================================="
echo "✅ Serenut OS Staging Deployment completed!"
echo "=================================================="
