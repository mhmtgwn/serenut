...
async function run() {
  ...
  try {
    ...
    await applyDomainMutation(client, 'sync-v4-company', {
      entity_type: 'product', entity_id: 'product-1', operation: 'UPSERT',
      payload: { name: 'Çay', price: 15, quantity: 20, category: 'İçecek', vat: 10 },
    });
    await applyDomainMutation(client, 'sync-v4-company', {
      entity_type: 'customer', entity_id: 'customer-1', operation: 'UPSERT',
      payload: { name: 'Ayşe', balance: 0, credit_limit: 0 },
    });
    const sale = {
      entity_type: 'sale', entity_id: 'sale-1', operation: 'UPSERT' as const,
      payload: {
    ...
    await applyDomainMutation(client, 'sync-v4-company', {
      entity_type: 'order', entity_id: 'order-1', operation: 'UPSERT',
      payload: {
    ...
    await applyDomainMutation(client, 'sync-v4-company', {
      entity_type: 'financial_transaction', entity_id: 'ledger-1', operation: 'UPSERT',
