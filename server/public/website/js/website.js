document.addEventListener('DOMContentLoaded', () => {
  ...
  const icons = { product:icon('M4 6h16M4 12h16M4 18h10'), plans:icon('M4 4h16v16H4zM8 9h8M8 13h5'), download:icon('M12 3v12m0 0 5-5m-5 5-5-5M5 21h14'), contact:icon('M4 5h16v14H4zM4 7l8 6 8-6'), login:icon('M10 17l5-5-5-5M15 12H3M15 4h5v16h-5'), register:icon('M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8ZM4 21a8 8 0 0 1 16 0M19 8v6M16 11h6'), account:icon('M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8ZM5 21a7 7 0 0 1 14 0') };
  ...
  const mobileAccountItems = profile
    ? `<a class="mobile-account-item" href="/app/">${icons.account}<span>Panel</span></a>`
    : `<button class="mobile-account-item" type="button" data-auth-window="login">${icons.login}<span>Giriş</span></button><button class="mobile-account-item mobile-register-item" type="button" data-auth-window="register">${icons.register}<span>Kayıt Ol</span></button>`;
  const headerMarkup = `<div class="container header-row"><a class="brand" href="/">Serenut</a><nav class="desktop-nav ${profile?'has-profile':'has-auth-actions'}" aria-label="Mobil ana navigasyon"><a href="/platform" ${currentPath==='/platform'?'aria-current="page"':''}>${icons.product}<span>Ürün</span></a><a href="/plans" ${currentPath==='/plans'?'aria-current="page"':''}>${icons.plans}<span>Planlar</span></a><a href="/downloads" ${currentPath==='/downloads'?'aria-current="page"':''}>${icons.download}<span>İndir</span></a><a href="/contact" ${currentPath==='/contact'?'aria-current="page"':''}>${icons.contact}<span>İletişim</span></a>${mobileAccountItems}</nav><div class="header-actions">${accountWidget}</div></div>`;
